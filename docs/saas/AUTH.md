...
1. User opens Sign in / Register in the header.
...
4. Header shows the signed-in email and Sign out.
5. Converter works the same when logged out (no login gate until pricing/quotas land).

Helpers live in `src/auth/supabase-client.js`: `getSupabaseClient`, `signUp`, `signIn`, `signInWithGoogle`, `signOut`, `getSession`, `onAuthStateChange`, `isAuthConfigured`.
...
Auth accounts live in `auth.users`. For a row you can see in **Table Editor**, use `public.profiles`:

- Migration SQL: [`supabase/migrations/20260910_profiles.sql`](../../supabase/migrations/20260910_profiles.sql)
- Run once in **Supabase → SQL Editor**
- Columns: `id` (FK to auth user), `email`, `display_name`, `plan` (`free` | `pro`), timestamps
- RLS: users can read/update their own row only
- Trigger `on_auth_user_created` inserts a profile on each new signup and backfills existing users

## What is out of scope for this phase

- Stripe / paid quotas
- Next.js or API routes
- Requiring login to convert
